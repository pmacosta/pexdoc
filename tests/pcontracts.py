...
# pylint: disable=C0103,C0111,E1101,E1121,F0401,W0212,W0232,W0611,W0612,W0613
...
def decfunc(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        """
        return ret_func(
            pexdoc.pcontracts._create_argument_value_pairs(
                func, *args, **kwargs
            )
        )
    return wrapper
...
def sample_func_global():
    ...
    tmp_global = pexdoc.pcontracts.get_exdesc()
    return tmp_global
...
def test_get_replacement_token():
    ...
    obj = pexdoc.pcontracts._get_replacement_token
    ref = 'argument_name'
...
def test_format_arg():
    ...
    fobj = pexdoc.pcontracts._format_arg
...
def test_format_arg_exceptions():
    ...
    obj = pexdoc.pcontracts._format_arg
...
def test_isexception():
    ...
    assert not pexdoc.pcontracts._isexception(str)
    assert not pexdoc.pcontracts._isexception(3)
    assert pexdoc.pcontracts._isexception(RuntimeError)


def test_parse_new_contract_args():
    """ Test _parse_new_contract_args function behavior """
    fobj = pexdoc.pcontracts._parse_new_contract_args
    # Validate *args
    ...
    assert pmisc.comp_list_of_dicts(
...
def test_register_custom_contracts():
    ...
    original_custom_contracts = copy.deepcopy(
        pexdoc.pcontracts._CUSTOM_CONTRACTS
    )
    fobj = pexdoc.pcontracts._register_custom_contracts
    key1 = 'contract_name'
    ...
    pexdoc.pcontracts._register_custom_contracts(
    ...
    pexdoc.pcontracts._CUSTOM_CONTRACTS = dict()
    fobj('test_contract1', 'my description')
    assert pexdoc.pcontracts._CUSTOM_CONTRACTS == {
        'test_contract1':{
            'default':{
                'num':0,
                'msg':'my description',
    ...
    #pexdoc.pcontracts._CUSTOM_CONTRACTS = dict()
    pexdoc.pcontracts._CUSTOM_CONTRACTS = copy.deepcopy(
...
def test_contract():
    ...
    pexdoc.pcontracts._CUSTOM_CONTRACTS = dict()
    @pexdoc.pcontracts.new_contract('Illegal number: *[number]*')
    def not_zero(number):
    ...
    def file_name_valid(name):
        ...
        return True
    @pexdoc.pcontracts.contract(number='int|float')
    def func1(number):
        return number
    @pexdoc.pcontracts.contract(number='not_zero')
    def func2(number):
        if number == 1:
            raise TypeError('Unfathomable')
        return number
    @pexdoc.pcontracts.contract(fname='str,file_name_valid', flag=bool)
    def func3(fname, fnumber, flag=False):
        return fname, fnumber
    @pexdoc.pcontracts.new_contract('Illegal number: unity')
    def not_one(number):
        exdesc = pexdoc.pcontracts.get_exdesc()
        if number == 1:
            raise ValueError(exdesc)
        return True
    @pexdoc.pcontracts.contract(fname='not_a_valid_contract')
    def func6(fname):
        return fname
    ...
    pexdoc.exh.set_exh_obj(pexdoc.exh.ExHandle())
    @pexdoc.pcontracts.contract(fname='str,file_name_valid')
    def func4(fname, fnumber):
        return fname, fnumber
    @pexdoc.pcontracts.contract(
        num='float|not_one', flag=bool, fudge='str|int'
    )
    ...
    func4('x', 5)
    func5(0)
    exdict = pexdoc.exh.get_exh_obj()._flatten_ex_dict()
    pexlist = list()
        ...
        pexlist.append(
            {
                'name':exkey[
                    exkey.rfind(
                        pexdoc.exh.get_exh_obj()._callables_separator
                    )+1:
                ],
                'type':exitem['type'],
                'msg':exitem['msg']
    ...
    ref = [
        {
            'name':'contract:tests.pcontracts.func5.flag_0',
            'type':RuntimeError, 'msg':'Argument `flag` is not valid'
        },
        {
            'name':'contract:tests.pcontracts.func5.fudge_0',
            'type':RuntimeError,
            'msg':'Argument `fudge` is not valid'
        },
        {
            'name':'contract:tests.pcontracts.func5.num_0',
            'type':RuntimeError,
            'msg':'Illegal number: unity'
        },
        {
            'name':'contract:tests.pcontracts.func4.fname_0',
            'type':OSError,
            'msg':'File name `*[file_name]*` not found'
        },
        {
            'name':'contract:tests.pcontracts.func4.fname_1',
            'type':RuntimeError,
            'msg':'The argument fname is wrong'
        }
    ]
    assert pmisc.comp_list_of_dicts(pexlist, ref)
    exmsg = 'The argument fname is wrong'
...
def test_enable_disable_contracts():
    ...
    """
    @pexdoc.pcontracts.contract(number=int)
    def func(number):
        return number
    assert not pexdoc.pcontracts.all_disabled()
    AI(func, 'number', number=None)
    pexdoc.pcontracts.disable_all()
    assert pexdoc.pcontracts.all_disabled()
...
def test_get_exdesc():
    ...
    def sample_func_local():
        ...
        tmp_local = pexdoc.pcontracts.get_exdesc()
        return tmp_local
    ...
    AE(pexdoc.pcontracts.get_exdesc, RuntimeError, exmsg)
...
def test_new_contract():
    ...
    @pexdoc.pcontracts.new_contract()
    def func1(name1):
        return name1, pexdoc.pcontracts.get_exdesc()
    ref = (
        'a',
    ...
    assert pexdoc.pcontracts._CUSTOM_CONTRACTS == ref
    pexdoc.pcontracts._CUSTOM_CONTRACTS = dict()
    @pexdoc.pcontracts.new_contract('Simple message')
    def func2(name2):
        return name2, pexdoc.pcontracts.get_exdesc()
    ref = (
        'bc',
    ...
    def func3(name3):
        return name3, pexdoc.pcontracts.get_exdesc()
    ref = (
        'def',
        {
            'ex1':(
