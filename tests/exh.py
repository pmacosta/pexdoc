...
# pylint: disable=C0103,C0111,C0302,C0413,E0611,E1101,F0401
# pylint: disable=R0201,R0204,R0205,R0903,R0912,R0914,R0915,R1717
# pylint: disable=W0108,W0122,W0123,W0212,W0612,W0631,W0640
...
# PyPI imports
import pmisc
from pmisc import AE, AI, AROPROP, CLDICTS, GET_EXMSG
import pytest
...
# Global variables
###
CTNAME = "tests.exh.TestExHandle"
PENG_FNAME = sys.modules["eng"].__file__
...
def test_star_exh_obj():
    ...
    AI(pexdoc.exh.set_exh_obj, "obj", obj=5)
    # set_exh_obj function
    exobj = pexdoc.exh.ExHandle()
    pexdoc.exh.set_exh_obj(exobj)
    assert id(pexdoc.exh.get_exh_obj()) == id(exobj)
    # del_exh_obj function
    pexdoc.exh.del_exh_obj()
    assert pexdoc.exh.get_exh_obj() is None
    # Test that nothing happens if del_exh_obj is called when there
    # is no global object handler set
    pexdoc.exh.del_exh_obj()
    new_exh_obj = pexdoc.exh.get_or_create_exh_obj()
    ...
    assert id(pexdoc.exh.get_or_create_exh_obj(True)) == id(new_exh_obj)
    assert not new_exh_obj._full_cname
    # Test passed arguments are correctly assigned
    pexdoc.exh.del_exh_obj()
    new_exh_obj = pexdoc.exh.get_or_create_exh_obj(True)
    assert new_exh_obj._full_cname
    pexdoc.exh.del_exh_obj()
    # get_or_create_exh_obj function
    obj = pexdoc.exh.get_or_create_exh_obj
    ...
    # exclude parameter
    pobj = pexdoc.pinspect.Callables([PENG_FNAME])
    new_exh_obj = pexdoc.exh.get_or_create_exh_obj(exclude=["eng"])
    new_exh_obj._exclude = ["eng"]
    pexdoc.exh.del_exh_obj()
    # callables_fname parameter
    with pmisc.TmpFile() as fname:
        pobj.save(fname)
        new_exh_obj = pexdoc.exh.get_or_create_exh_obj(callables_fname=fname)
        assert pobj == new_exh_obj._callables_obj
    pexdoc.exh.del_exh_obj()


@pytest.mark.parametrize(
    "arg, ref", [(RuntimeError, "RuntimeError"), (OSError, "OSError")]
)
def test_ex_type_str(arg, ref):
    """Test _ex_type_str() function behavior."""
    assert pexdoc.exh._ex_type_str(arg) == ref
...
class TestExHandle(object):
    ...
    def test_init(self):
        ...
        exobj = pexdoc.exh.ExHandle(False, [])
        assert not exobj._full_cname
        assert exobj._exclude == []
        assert exobj._exclude_list == []
        exobj = pexdoc.exh.ExHandle(True, None)
        assert exobj._full_cname
        assert exobj._exclude is None
        assert exobj._exclude_list == []
        exobj = pexdoc.exh.ExHandle(exclude=["pexdoc.exh"])
        assert exobj._exclude == ["pexdoc.exh"]
        assert exobj._exclude_list == [
            sys.modules["pexdoc.exh"].__file__.replace(".pyc", ".py")
        ]
        pobj = pexdoc.pinspect.Callables([PENG_FNAME])
        with pmisc.TmpFile() as fname:
            pobj.save(fname)
            exobj = pexdoc.exh.ExHandle(callables_fname=fname)
    ...
    def test_add_exception_exceptions(self, full_cname, args, extype, exname):
        ...
        obj = pexdoc.exh.ExHandle(full_cname).add_exception
        AE(obj, extype, exname, *args)
        # These should not raise an exception
        obj = pexdoc.exh.ExHandle(full_cname)
    ...
    def test_add_exception(self):
        ...
        @pexdoc.pcontracts.contract(text=str)
        @prop_decorator
        def func2(text):
        ...
        iftxt = "def func15(exobj):" "   aobj(emsg(15), TypeError, mmsg(15))"
        gmap = locals()
        ...
        # Assertions
        exobj = pexdoc.exh.ExHandle(full_cname=True, exclude=["_pytest", "tests.exh"])
        assert exobj.exceptions_db == []
        # Generate all possible combinations of full_cname and exclude options
        #                       full_cname     exclude
        combinations = product([True, False], [None, ["_pytest"]])
        for full_cname, exclude in combinations:
            exobj = pexdoc.exh.ExHandle(full_cname=full_cname, exclude=exclude)
            ...
            for exname in cdb:
                ...
                match_dict = dict([(key, value) for key, value in zip(fkeys, iobj)])
            ...
            exobj = pexdoc.exh.ExHandle(full_cname=True, exclude=["_pytest"])
            ...
            frobj = sys._getframe(0)
            cname = "pexdoc.exh.sys._getframe"
            ...
            erec["function"] = [exobj.decode_call(call) for call in erec["function"]]
        ...
        pexdoc.exh.set_exh_obj(
            pexdoc.exh.ExHandle(full_cname=True, exclude=["_pytest"])
        )
        _ = eng.peng(15, 3, False)
        cdb = pexdoc.exh.get_exh_obj()._flatten_ex_dict()
        for item in cdb.values():
            assert item["function"][0]
        pexdoc.exh.set_exh_obj(
            pexdoc.exh.ExHandle(full_cname=True, exclude=["_pytest", "eng"])
        )
        _ = eng.peng(15, 3, False)
        cdb = pexdoc.exh.get_exh_obj()._flatten_ex_dict()
        for item in cdb.values():
    ...
    def test_raise_exception_if_exceptions(self):
        ...
        def check_str(exobj, slist, llist):
            ...
            entry = cdb_int[list(cdb_int.keys())[0]]
            assert [exobj.decode_call(item) for item in entry["function"]] == ref
        ...
        for exname, erec in cdb.items():
            mname = "exh.test_raise_exception.func3"
        ...
        exobj = pexdoc.exh.ExHandle(full_cname=True)
        ...
        def func_base(exobj, cond):
            ...
            exobj.raise_exception_if(exname="multi_path_exception", condition=cond)
    ...
    def test_exceptions_db(self):
        ...
        for full_cname in [True, False]:
            ...
            def func5(exobj):
                exobj.add_exception(
                    "my_exception2", ValueError, "This is exception #2, *[result]*"
                )
                exobj.add_exception("my_exception3", TypeError, "This is exception #3")

            exobj = pexdoc.exh.ExHandle(full_cname)
            func4(exobj)
            func5(exobj)
            # Actual tests
    ...
    def test_save_callables(self):
        ...
        obj1 = pexdoc.pinspect.Callables([PENG_FNAME])
        with pmisc.TmpFile() as fname1:
            with pmisc.TmpFile() as fname2:
                ...
                obj2 = pexdoc.exh.ExHandle(callables_fname=callables_fname1)
    ...
    def test_callables_db(self):
        ...
        # Actual tests
        exobj = func6(pexdoc.exh.ExHandle())
    ...
    def test_str(self):
        ...
        for full_cname in [True, False]:
            ...
            def func8(exobj):
                exobj.add_exception(
                    "my_exception8", ValueError, "This is exception #8, *[fname]*"
                )
            ...
            str_out = list()
            cname = "exh.TestExHandle.test_str"
            for str_element in str_in:
                str_list = str_element.split("\n")
                if str_list[0].endswith("/my_exception7"):
                    str_list[0] = "Name    : {0}.func7/my_exception7".format(cname)
                elif str_list[0].endswith("/my_exception8"):
                    str_list[0] = "Name    : {0}.func8/my_exception8".format(cname)
                elif str_list[0].endswith("/my_exception9"):
                    str_list[0] = "Name    : {0}.func8/my_exception9".format(cname)
    ...
    def test_copy(self):
        ...
        source_obj = pexdoc.exh.ExHandle(full_cname=True)
        funca(source_obj)
        funcb(source_obj)
        obj = Clsc(source_obj)
        obj.value = 5
    ...
    def test_multiple_paths_to_same_exception(self):  # noqa: D202
        """Test different paths to single exception do not overwrite each other."""
        ...
        assert exdb[0]["name"].endswith("{0}/{0}.funca/{0}.exdef".format(cname))
        assert exdb[1]["name"].endswith("{0}/{0}.funcb/{0}.exdef".format(cname))
        str_in = pmisc.flatten_list(
        ...
        assert str_in[3].endswith(fstring.format("a")) or str_in[3].endswith(
            fstring.format("b")
        )
        assert str_in[4].startswith(" " * 10)
        assert str_in[4].endswith(
            fstring.format("a" if str_in[3].endswith(fstring.format("b")) else "b")
        )
    ...
    def test_add(self):
        ...
        def comp_objs(obj):
            ...
            nref = [
                ...
                "copy_exception_3",
                "copy_exception_4",
                "contract:eng.peng.frac_length_0",
                "contract:eng.peng.number_0",
                "contract:eng.peng.rjust_0",
            ]
            alist = [
            ...
            for key, value in obj._flatten_ex_dict().items():
                ...
                if name == "copy_exception_1":
                    assert value["function"] == ["0/1", "0/4/1"]
                    assert value["raised"] == [True, False]
                elif name == "copy_exception_2":
                    assert value["function"] == ["0/3"]
                    assert value["raised"] == [False]
                elif name == "copy_exception_3":
                    assert value["function"] == ["0/2"]
                    assert value["raised"] == [False]
                elif name == "copy_exception_4":
        ...
        obj1 = pexdoc.exh.ExHandle(_copy=True)
        obj2 = pexdoc.exh.ExHandle(_copy=True)
        # Incompatible types
        with pytest.raises(TypeError) as excinfo:
            obj1 + 5
        msg = "Unsupported operand type(s) for +: pexdoc.ExHandle and int"
        ...
        obj1 = pexdoc.exh.ExHandle(full_cname=True, exclude=exclude_list)
        obj2 = pexdoc.exh.ExHandle(full_cname=True, exclude=exclude_list)

        def func1(exhobj, extra=False, rex=False):
            exhobj.add_exception("copy_exception_1", TypeError, "Copy exception #1")
            if extra:
                exhobj.add_exception("copy_exception_4", TypeError, "Copy exception #4")
            exhobj.raise_exception_if("copy_exception_1", rex)

        def func2(exhobj):
            exhobj.add_exception("copy_exception_2", RuntimeError, "Copy exception #2")

        def func3(exhobj):
            exhobj.add_exception("copy_exception_3", ValueError, "Copy exception #3")
            pexdoc.exh.del_exh_obj()
            pexdoc.exh.get_or_create_exh_obj(full_cname=True, exclude=exclude_list)
            eng.peng(1, 3, False)
            exhobj += pexdoc.exh.get_exh_obj()
            pexdoc.exh.del_exh_obj()

        def func4(exhobj):
            """Create an different path to func1."""
            func1(exhobj)

        # Test __add__
        # obj1 will have exceptions 1 and 3 plus the ones from peng
        func1(obj1)
        func3(obj1)
        # obj2 will have exceptions 1, 2 and 4 plus the ones from peng
        # via two paths
        try:
            func1(obj2, True, True)
        except TypeError:
            pass
        func2(obj2)
        func3(obj2)
        ...
        assert obj2_ref == obj2
        comp_objs(obj1)
        # Test behavior with no full_cname and __add__
        ...
        # Test behavior with no full_cname and __iadd__
        pexdoc.set_exh_obj(pexdoc.ExHandle())
        mynumber(1)
        mystring("Tom")
        obj1 = copy.copy(pexdoc.get_exh_obj())
    ...
    def test_eq(self):
        ...
        pexdoc.exh.get_or_create_exh_obj()
        # Trace some exceptions
        eng.peng(100, 3, True)
        obj1 = pexdoc.exh.get_exh_obj()
        obj2 = copy.copy(obj1)
        assert obj1 == obj2
        assert obj1 != 5
