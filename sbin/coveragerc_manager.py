...
def main(argv):
    ...
    source_dir = os.path.join(site_pkg_dir, 'pexdoc')
    output_file_name = os.path.join(
        site_pkg_dir, 'pexdoc', '.coveragerc_{0}_{1}'.format(env, interp)
    )
    coverage_file_name = os.path.join(
        site_pkg_dir, 'pexdoc', '.coverage_{0}'.format(interp)
    )
    conf_file = []
    ...
    if mode_flag == '1':
        ...
        start_flag = True
        # Include modules
        source_files = get_source_files(os.path.join(site_pkg_dir, 'pexdoc'))
