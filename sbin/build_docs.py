...
# pylint: disable=C0111,C0411,F0401,R0914,R0915,W0141
...
def generate_top_level_readme(pkg_dir):
    ...
    for line in lines:
        ...
        if autofunction:
            ...
            line = line.replace(
                ':py:mod:`{label} <pexdoc.{mname}>`'.format(
...
if __name__ == "__main__":
    ...
    PARSER = argparse.ArgumentParser(
        description='Build pexdoc package documentation'
    )
    PARSER.add_argument(
        '-d', '--directory',
        help='specify source file directory (default ../pexdoc)',
        type=valid_dir,
        nargs=1,
        default=[os.path.join(PKG_DIR, 'pexdoc')]
    )
